...
- Некоторые деплои AdminAntizapret дописывают в имя файла хост панели: `antizapret-<name>-(vpn1.example.com).ovpn`. По умолчанию `antizapret_client.py` сам берёт этот суффикс из хоста `base_url` — обычно ничего настраивать не нужно. Если у вашего сервера имена файлов другие — переопределите (или отключите пустой строкой) полем `filename_suffix` у конкретного сервера в `vpn_profiles.servers`.
